...
export async function checkUserBalance(phoneNumberId: string): Promise<number> {
  const sql = `
...
export async function deductTokens(phoneNumberId: string, tokensUsed: number): Promise<void> {
  const sql = `
...
export async function logTokenUsage(phoneNumberId: string, endUserPhone: string, input: number, output: number) {
